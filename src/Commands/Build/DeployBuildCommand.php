...
class DeployBuildCommand extends AbstractCommand
{
    ...
    public function execute(array $args, Application $app): int
    {
        ...
        $copied = $this->copyProjectFiles($this->getBasePath(), $outputPath, $output);
    ...
    private function copyProjectFiles(string $source, string $dest, string $excludeDir): int
    {
        $copied = 0;
        ...
        foreach ($iterator as $item) {
            ...
            // Prevent infinite recursion by excluding output directory
            if ($relativePath === $excludeDir || str_starts_with($relativePath, $excludeDir . DIRECTORY_SEPARATOR) || str_starts_with($relativePath, $excludeDir . '/')) {
                continue;
            }
